...
static int FindConfigFile(_Pre_writable_size_(MAX_PATH_SIZE) char path[MAX_PATH_SIZE])
{
    ...
    {
        Strlcpy(path, "./.omigenrc", MAX_PATH_SIZE);
    ...
    if (home)
    {
        ...
        Strlcat(path, "/.omigenrc", MAX_PATH_SIZE);
    ...
    {
        Strlcpy(path, GetPath(ID_DESTDIR), MAX_PATH_SIZE);
        Strlcat(path, "/", MAX_PATH_SIZE);
        Strlcat(path, GetPath(ID_SYSCONFDIR), MAX_PATH_SIZE);
        Strlcat(path, "/omigen.conf", MAX_PATH_SIZE);
...
int MI_MAIN_CALL main(int argc, const char** argv)
{
    ...
    schemafile = GetPath(ID_SCHEMAFILE);
