...
namespace ut {
    ...
    void testCompleted(const char* /*name*/)
    {
        ...
        {
            Zprintf(ZT("%s"), scs(s_lastTestFailed ? "X" : "*"));
...
#pragma prefast (pop)
#endif
        {
            ...
            if (i + 1 == argc)
            {
                Fzprintf(stderr, ZT("Missing option argument for -n"));
                Usage();
                exit(1);
            }
        ...
        else if (strcmp(argv[i], "-xml") == 0)
        {
            ...
            if (i + 1 == argc)
            {
                Fzprintf(stderr, ZT("Missing option argument for -xml"));
                Usage();
                exit(1);
            }
        ...
        else if (strcmp(argv[i], "-a") == 0)
        {
            ...
            if (i + 1 == argc)
            {
                Fzprintf(stderr, ZT("Missing option argument for -a"));
                Usage();
                exit(1);
            }
        ...
        {
            Fzprintf(stderr, ZT("unknown option: %s"), scs(argv[i]));
            Usage();
            exit(1);
        }
        else
...
<Failures>0</Failures>\
    ...
    {
        Fzprintf(stderr, ZT("unable to open file %s\n"),
...
s_summary = true;
    ...
    if (s_summary)
    {
        Zprintf(ZT("Completed %d test(s)"), s_tests);
