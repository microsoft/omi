...
/* Parse a numeric field of a fixed width.  If _asterisks_ is true, the field may
   optionally contain all asterisks.  If the field is asterisks, the numeric value
   returned is zero */
...
void yyerrorf(int id, const char *format, ...)
{
    ...
    if (wformat)
    {
        ...
        if (state.path)
        {
            n = _snwprintf_w(buf, MI_COUNT(buf), _TRUNCATE,
...
#endif
    {
        ...
        MofPtrArray_Append(&state.errors, MOF_Strdup(&state.heap, buf));
...
void yywarnf(int id, const char *format, ...)
{
...
#endif
    {
        ...
        MofPtrArray_Append(&state.warnings, MOF_Strdup(&state.heap, buf));
    }
}
    ...
    void** value)
{
    ...
    *value = NULL;
    ...
    /* Early out for a null initializer */
    if (self->size == 0)
        return 0;
...
int AddQualifierDeclaration(MI_QualifierDecl* qd)
{
    ...
    /* Add the declaration */
    MofPtrArray_Append((MofPtrArray*)&state.qualifierDecls, qd);
    return 0;
}
...
int AddClassDecl(MI_ClassDecl* qd)
{
    ...
    {
        yywarnf(ID_CLASS_ALREADY_DEFINED, "class already declared: \"%s\"",
            MI_GET_SAFE_PRINTF_STRING(qd->name));
        return -1;
    }

    /* Add the declaration */
    MofPtrArray_Append((MofPtrArray*)&state.classDecls, qd);
    return 0;
}

MI_Uint32 GetFlavorFlag(char* Flavor)
{
    if (Strcasecmp(Flavor, "EnableOverride") == 0)
    {
        return MI_FLAG_ENABLEOVERRIDE;
    }
    if (Strcasecmp(Flavor, "DisableOverride") == 0)
    {
        return MI_FLAG_DISABLEOVERRIDE;
    }
    if (Strcasecmp(Flavor, "Restricted") == 0)
    {
        return MI_FLAG_RESTRICTED;
    }
    if (Strcasecmp(Flavor, "ToSubClass") == 0)
    {
        return MI_FLAG_TOSUBCLASS;
    }
    if (Strcasecmp(Flavor, "ToInstance") == 0)
    {
        return MI_FLAG_TOINSTANCE;
    }
    if (Strcasecmp(Flavor, "Translatable") == 0)
    {
        return MI_FLAG_TRANSLATABLE;
    }
    if (Strcasecmp(Flavor, "NotToSubClass") == 0 || Strcasecmp(Flavor, "NotToInstance") == 0)
    {                                   // these are defaults; do nothing
        return 0;
    }
    return MI_FLAG_NULL;
}

MI_Uint32 GetScopeFlag(char* Scope)
{
    if (Strcasecmp(Scope, "Class") == 0)
    {
        return MI_FLAG_CLASS;
    }
    if (Strcasecmp(Scope, "Association") == 0)
    {
        return MI_FLAG_ASSOCIATION;
    }
    if (Strcasecmp(Scope, "Indication") == 0)
    {
        return MI_FLAG_INDICATION;
    }
    if (Strcasecmp(Scope, "Property") == 0)
    {
        return MI_FLAG_PROPERTY;
    }
    if (Strcasecmp(Scope, "Reference") == 0)
    {
        return MI_FLAG_REFERENCE;
    }
    if (Strcasecmp(Scope, "Method") == 0)
    {
        return MI_FLAG_METHOD;
    }
    if (Strcasecmp(Scope, "Parameter") == 0)
    {
        return MI_FLAG_PARAMETER;
    }
    if (Strcasecmp(Scope, "Any") == 0)
    {
        return MI_FLAG_ANY;
    }
    return MI_FLAG_NULL;
    ...
    size_t numQualifiers)
{
    ...
    for (i = 0; i < numQualifiers; i++)
    {
            ...
            Strcasecmp(q->name, "EmbeddedInstance") == 0)
        {
            ...
            if (MofPtrArray_Append(
                (MofPtrArray*)(void*)&state.embeddedInstanceList, p) != 0)
            {
                yyerrorf(ID_OUT_OF_MEMORY, "out of memory");
                return 0;
            }
    ...
    MI_Uint32* numQualifiers_)
{
    ...
    for (i = 0; i < numInherited; i++)
    {
        ...
        {
            if (MofPtrArray_Append((MofPtrArray*)(void*)&qualifierList, q) != 0)
            {
                yyerrorf(ID_OUT_OF_MEMORY, "out of memory");
                return -1;
            }
    ...
    MI_ClassDecl* cd)
{
    ...
    if (cd->superClass)
    {
        ...
        {
            if (MofPtrArray_Append((MofPtrArray*)(void*)&propertySet,
    ...
    for (i = 0; i < cd->numProperties; i++)
    {
        ...
        if (pos == MOF_NOT_FOUND)
        {
            ...
            MofPtrArray_Append((MofPtrArray*)(void*)&propertySet, pd);
        }
        else
        {
            MI_Qualifier** qualifiers;
    ...
    MI_ClassDecl* cd)
{
    ...
    if (cd->superClass)
    {
        ...
        {
            if (MofPtrArray_Append((MofPtrArray*)(void*)&methodList,
                super->methods[i]) != 0)
            {
                yyerrorf(ID_OUT_OF_MEMORY, "out of memory");
                return -1;
    ...
    for (i = 0; i < cd->numMethods; i++)
    {
        ...
        if (pos == MOF_NOT_FOUND)
        {
            ...
            MofPtrArray_Append((MofPtrArray*)(void*)&methodList, md);
        }
        else
        {
            MI_Qualifier** qualifiers;
...
int FinalizeClass(MI_ClassDecl* cd)
{
    /*
        ...
        - derived class may not introduce new keys or
...
int FinalizeInstance(MI_InstanceDecl* id)
{
    ...
    for (i = 0; i < id->numProperties; i++)
    {
        ...
        /* Promote instance property (to the type given by class property)
           if the instance assigned a value */
...
MI_Uint32 PropagateFlavors(MI_Uint32 flavor, MI_Uint32 baseFlavor)
{
    ...
    /* MI_FLAG_TOINSTANCE | MI_FLAG_RESTRICTED */
    if (!(r & MI_FLAG_TOINSTANCE) && !(r & MI_FLAG_RESTRICTED))
    {
        if (baseFlavor & MI_FLAG_RESTRICTED)
            r |= MI_FLAG_RESTRICTED;
    }
...
{
    /* Add the declaration */
    MofPtrArray_Append((MofPtrArray*)&state.instanceDecls, id);
    return 0;
}
