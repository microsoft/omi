...
void PopulateCIMError(CIM_Error *error, MI_Char* message, MI_Uint32 statusCode, MI_Char* errorSource, MI_ErrorCategory errorType, MI_Char* messageID)
{
...
bool PreCheckEmployee(Employee_Self* self, CIM_Error *error, MI_Context* context)
{
	...
	if(self->instanceStore.size() <= 0)
	{
		...
		sprintf(errorSource, MI_T("%s"), MI_T("PreCheckEmployee"));
		sprintf(messageID, MI_T("%s"), MI_T("Message ID"));
	...
	const MI_PropertySet* propertySet)
{
	...
	for(it = self->instanceStore.begin(); it != self->instanceStore.end(); it++)
	{
		//if(_wcsicmp((*it)->SSN.value, instanceName->SSN.value) == 0)
		if(strcasecmp((*it)->SSN.value, instanceName->SSN.value) == 0)
		{
			result = Employee_Post((*it), context);
	...
	const Employee* newInstance)
{
	...
	for(it = self->instanceStore.begin(); it != self->instanceStore.end(); it++)
	{
		//if(_wcsicmp((*it)->SSN.value, newInstance->SSN.value) == 0) 
		if(strcasecmp((*it)->SSN.value, newInstance->SSN.value) == 0) 
		{
	...
	if(newInstance->dateOfBirth.exists == 1) 
	{
		if(newInstance->dateOfBirth.value.isTimestamp == 0)
		{
			...
			sprintf(message, MI_T("%s"), MI_T("Cannot create a date of birth with interval type date"));
			sprintf(errorSource, MI_T("%s"), MI_T("Employee_CreateInstance"));
			sprintf(messageID, MI_T("%s"), MI_T("MessageID"));
	...
	else
	{
		...
		sprintf(message, MI_T("%s"), MI_T("Instance Cloning failed"));
		sprintf(errorSource, MI_T("%s"), MI_T("Employee_CreateInstance"));
		sprintf(messageID, MI_T("%s"), MI_T("MessageID"));
	...
	const MI_PropertySet* propertySet)
{
	...
	for(it = self->instanceStore.begin(); it != self->instanceStore.end(); it++)
	{
		//if(_wcsicmp((*it)->SSN.value, modifiedInstance->SSN.value) == 0)
		if(strcasecmp((*it)->SSN.value, modifiedInstance->SSN.value) == 0)
		{
	...
	if(foundInstance != MI_TRUE)
	{
		...
		sprintf(message, MI_T("%s"), MI_T("Instance not found for specified key"));
		sprintf(errorSource, MI_T("%s"), MI_T("Employee_ModifyInstance"));
		sprintf(messageID, MI_T("%s"), MI_T("MessageID"));
	...
	const Employee* instanceName)
{
	...
	if(foundInstance != MI_TRUE)
	{
		...
		sprintf(message, MI_T("%s"), MI_T("Instance not found for specified key"));
		sprintf(errorSource, MI_T("%s"), MI_T("Employee_DeleteInstance"));
		sprintf(messageID, MI_T("%s"), MI_T("MessageID"));
