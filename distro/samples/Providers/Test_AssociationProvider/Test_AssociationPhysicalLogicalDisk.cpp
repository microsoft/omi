...
MI_Result PostPhysicalToLogicalAssociation(MI_Context* context, const Test_PhysicalDisk* instanceName, MI_Boolean isReference)
{
	...
	{	
#if(MI_CHAR_TYPE == 1)
		if(strcasecmp(instanceName->serialNumber.value, MI_T("10000")) == 0)
#else
		if(wcasecmp(instanceName->serialNumber.value, MI_T("10000")) == 0)
#endif
		{
			for(MI_Uint32 i = 0; i < logicalDiskForAssocInstanceStore.size(); i++)
			{
				if(logicalDiskForAssocInstanceStore[i]->diskID.value == 0 && (strcasecmp(logicalDiskForAssocInstanceStore[i]->driveLetter.value, MI_T("C")) == 0))
				{
				...
				if(logicalDiskForAssocInstanceStore[i]->diskID.value == 1 && (strcasecmp(logicalDiskForAssocInstanceStore[i]->driveLetter.value, MI_T("D")) == 0) )
				{
					if(isReference == MI_TRUE)
					{
						Test_AssociationPhysicalLogicalDisk assocLogical;
...
#if(MI_CHAR_TYPE == 1)
		else if(strcasecmp(instanceName->serialNumber.value, MI_T("10001")) == 0)
#else
		else if(wcasecmp(instanceName->serialNumber.value, MI_T("10001")) == 0)
#endif
		{
			for(MI_Uint32 i = 0; i < logicalDiskForAssocInstanceStore.size(); i++)
			{
				if(logicalDiskForAssocInstanceStore[i]->diskID.value == 2 && (strcasecmp(logicalDiskForAssocInstanceStore[i]->driveLetter.value, MI_T("E")) == 0) )
				{
					if(isReference == MI_TRUE)
					{
						Test_AssociationPhysicalLogicalDisk assocLogical;
...
MI_Result PostLogicalToPhysicalAssociation(MI_Context* context, const Test_LogicalDisk* instanceName, MI_Boolean isReference)
{
	...
	{	
#if(MI_CHAR_TYPE == 1)
		if( (instanceName->diskID.value == 0 && (strcasecmp(instanceName->driveLetter.value, MI_T("C")) == 0)) ||  (instanceName->diskID.value == 1 && (strcasecmp(instanceName->driveLetter.value, MI_T("D")) == 0)))
#else
		if( (instanceName->diskID.value == 0 && (wcasecmp(instanceName->driveLetter.value, MI_T("C")) == 0)) ||  (instanceName->diskID.value == 1 && (wcasecmp(instanceName->driveLetter.value, MI_T("D")) == 0)))
#endif
		{
			for(MI_Uint32 i = 0; i < physicalDiskForAssocInstanceStore.size(); i++)
			{
				if(strcasecmp(physicalDiskForAssocInstanceStore[i]->serialNumber.value, MI_T("10000")) == 0)
				{
					if(isReference == MI_TRUE)
					{
...
#if(MI_CHAR_TYPE == 1)
		else if(instanceName->diskID.value == 2 && (strcasecmp(instanceName->driveLetter.value, MI_T("E")) == 0))
#else
		else if(instanceName->diskID.value == 2 && (wcasecmp(instanceName->driveLetter.value, MI_T("E")) == 0))
#endif
		{
			for(MI_Uint32 i = 0; i < physicalDiskForAssocInstanceStore.size(); i++)
			{
				if(strcasecmp(physicalDiskForAssocInstanceStore[i]->serialNumber.value, MI_T("10001")) == 0)
				{
					if(isReference == MI_TRUE)
					{
						Test_AssociationPhysicalLogicalDisk assocPhysical;
