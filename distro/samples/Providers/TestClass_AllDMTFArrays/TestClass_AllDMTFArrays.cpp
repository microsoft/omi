...
std::wstring providerName = L"TestClass_AllDMTFArrays";
#endif
...
{
	MI_Boolean* boolArray = (MI_Boolean *) malloc (sizeof(MI_Boolean) * count);
...
{
	MI_Uint8* uint8Array = (MI_Uint8 *) malloc (sizeof(MI_Uint8) * count);
...
{
	MI_Sint8* sint8Array = (MI_Sint8 *) malloc (sizeof(MI_Sint8) * count);
...
{
	MI_Uint16* uint16Array = (MI_Uint16 *) malloc (sizeof(MI_Uint16) * count);
...
{
	MI_Sint16* sint16Array = (MI_Sint16 *) malloc (sizeof(MI_Sint16) * count);
...
{
	MI_Uint32* uint32Array = (MI_Uint32 *) malloc (sizeof(MI_Uint32) * count);
...
{
	MI_Sint32* sint32Array = (MI_Sint32 *) malloc (sizeof(MI_Sint32) * count);
...
{
	MI_Uint64* uint64Array = (MI_Uint64 *) malloc (sizeof(MI_Uint64) * count);
...
{
	MI_Sint64* sint64Array = (MI_Sint64 *) malloc (sizeof(MI_Sint64) * count);
...
{
	MI_Real32* real32Array = (MI_Real32 *) malloc (sizeof(MI_Real32) * count);
...
{
	MI_Real64* real64Array = (MI_Real64 *) malloc (sizeof(MI_Real64) * count);
...
{
	MI_Char16* char16Array = (MI_Char16 *) malloc (sizeof(MI_Char16) * count);
...
{
	MI_Char ** stringArray = (MI_Char **) malloc (sizeof(MI_Char*) * count);

	for(MI_Uint32 i=0; i < count; i++)
	{
		stringArray[i] = (MI_Char *) malloc (sizeof(MI_Char) * (100) );
		memset((void *)stringArray[i], 0, sizeof(MI_Char) * 100);
		sprintf(stringArray[i], MI_T("Test String %d"), i);
	}
...
{
	MI_Datetime *datetimeArray = (MI_Datetime *) malloc (sizeof(MI_Datetime) * count);
...
{
	TestClass_ForEmbedded **instanceArray = (TestClass_ForEmbedded **) malloc (sizeof(TestClass_ForEmbedded*) * count);
...
{
	MI_Instance **objectArray = (MI_Instance **) malloc (sizeof(MI_Instance*) * count);
	...
	MI_Context* context)
{
	*self = new TestClass_AllDMTFArrays_Self;
	...
	if(faultReader.CheckForFaultInjection(ProviderXMLLogger::scope::PROVIDER_LOAD, result)) 
	{
		...
		POST_ERROR(context, result, MI_RESULT_TYPE_MI, MI_T("User generated error"));
	}

	wchar_t wzProfile[MAX_PATH]; 

	if(!GetEnvironmentVariable(L"UserProfile", wzProfile, MAX_PATH))
	{
		//MI_PostResult(context, MI_RESULT_FAILED);		
		POST_ERROR(context, MI_RESULT_FAILED, MI_RESULT_TYPE_MI, MI_T("Failed to get environment variable UserProfile"));
	}
	...
	if(!logger.Initialize(sStream.str(), L"TestClass_AllDMTFArrays_Log.xml", providerName ))
	{
		//MI_PostResult(context, MI_RESULT_ACCESS_DENIED);		
		POST_ERROR(context, MI_RESULT_ACCESS_DENIED, MI_RESULT_TYPE_MI, MI_T("Cannot create log file"));
	}
	...
	const MI_PropertySet* propertySet)
{
	...
	if(faultReader.CheckForFaultInjection(ProviderXMLLogger::scope::MODIFY_INSTANCE, result)) 
	{
		//MI_PostResult(context, result);
		POST_ERROR(context, result, MI_RESULT_TYPE_MI, MI_T("User generated error"));
	}
	...
	result = MI_GetCustomOption(context, L"ReportFlagsOfPropertyToModify", &reportFlagsOfPropertyToModifyType, &reportFlagsOfPropertyToModifyValue);
	if ((result == MI_RESULT_OK) && (reportFlagsOfPropertyToModifyType == MI_STRING))
	{
		...
		if(result != MI_RESULT_OK)
		{
			logger.WriteResult(ProviderXMLLogger::scope::INVOKE_METHOD, L"ERROR from MI_Instance_GetElementAt(... flagsToReport ...)", result);
			POST_ERROR(context, result, MI_RESULT_TYPE_MI, MI_T("Could not get flags to report"));
		}

		MI_Char warningMessage[100];
		HRESULT hr = StringCchPrintfW(warningMessage, 100, L"ReportFlags: %s: 0x%08x", nameOfPropertyToReport, flagsToReport);
		if (FAILED(hr))
		{
			result = MI_RESULT_FAILED;
			logger.WriteResult(ProviderXMLLogger::scope::INVOKE_METHOD, L"ERROR from StringCchPrintfW(... flagsToReport ...)", result);
		...
		if(result != MI_RESULT_OK)
		{
			logger.WriteResult(ProviderXMLLogger::scope::INVOKE_METHOD, L"ERROR from MI_WriteMessage(... flagsToReport ...)", result);
	...
	const TestClass_AllDMTFArrays_SetStringArray* in)
{
	...
	result = MI_GetCustomOption(context, L"ReportFlagsOfInParameter", &reportFlagsOfInParameterType, &reportFlagsOfInParameterValue);
	if (result == MI_RESULT_OK)
	{
		MI_Char* nameOfPropertyToReport = L"StringArray";

		MI_Uint32 flagsToReport;
		result = MI_Instance_GetElement(&(in->__instance), nameOfPropertyToReport, NULL, NULL, &flagsToReport, NULL);
		if(result != MI_RESULT_OK)
		{
			logger.WriteResult(ProviderXMLLogger::scope::INVOKE_METHOD, L"ERROR from MI_Instance_GetElementAt(... flagsToReport ...)", result);
			POST_ERROR(context, result, MI_RESULT_TYPE_MI, MI_T("Could not get flags to report"));
		...
		MI_Char warningMessage[100];
		HRESULT hr = StringCchPrintfW(warningMessage, 100, L"ReportFlags: StringArray: 0x%08x", flagsToReport);
		if (FAILED(hr))
		{
			result = MI_RESULT_FAILED;
			logger.WriteResult(ProviderXMLLogger::scope::INVOKE_METHOD, L"ERROR from StringCchPrintfW(... flagsToReport ...)", result);
