...
#if defined(CONFIG_COMPILER_GNU) && !defined(__USE_GNU)
# define __USE_GNU
#endif
...
int AuthenticateUser(const char* user, const char* password)
{
    ...
    {
        LOGE((ZT("failed to read result from child; errno (%d)"), errno));
        r = -1;
        goto done;
    }
...
int LookupUser(const char* user, uid_t* uid, gid_t* gid)
{
    ...
    if (0 != getpwnam_r(user, &pwd, buf, sizeof(buf), &ppwd) ||
        !ppwd)
    {
        LOGW((ZT("getpwnam_r filed, errno %d"), errno));
        return -1;
    }
...
#if defined(CONFIG_ENABLE_DIR_CREATION)
    ...
    const gid_t gid)
{
    ...
    strncat(path, userName, sizeof(path)-1-strlen(path));
    ...
    if (mkdir(path, 0700) == 0)
    {
        ...
        {
            LOGI((ZT("Created dir: \"%s\""), path));
        }
        else
        {
            LOGW((ZT("Failed to change owner for dir: \"%s\", errno=%d"), path, errno));
        }
    }
    else
    {
        ...
        if (errno != EEXIST)
        {
            LOGW((ZT("Failed to create dir: \"%s\", errno=%d"), path, errno));
        }
    }
}
#endif // defined(CONFIG_ENABLE_DIR_CREATION)
...
int SetUser(uid_t uid, gid_t gid)
{
    ...
    {
        LOGW((ZT("getpwuid_r failed: errno (%d)"), errno));
        return -1;
    }
    ...
    //
    // We'll need to rework this when we move away from SCX-specific OMI.
    if (uid != 0)
    {
        _createDir("/var/opt/microsoft/scx/lib/state/", pw->pw_name, uid, gid);
        _createDir("/var/opt/microsoft/scx/log/", pw->pw_name, uid, gid);
    }
#endif // defined(CONFIG_ENABLE_DIR_CREATION)

    if (setgid(gid) != 0)
    {
        LOGW((ZT("setgid failed: errno (%d)"), errno));
        return -1;
    }
    ...
    {
        LOGW((ZT("initgroups failed: errno (%d)"), errno));
        return -1;
    }

    if (setuid(uid) != 0)
    {
        LOGW((ZT("setuid failed: errno (%d)"), errno));
        return -1;
    }
...
int CreateAuthFile(uid_t uid, char* content, size_t size, char path[MAX_PATH_SIZE])
{
...

    {
        ...
        if (fd == -1)
        {
            LOGW((ZT("failed to create auth file %s"), scs(path)));
            return -1;
        }
        ...
        {
            LOGW((ZT("failed to chown auth file %s"), scs(path)));
            close(fd);
            unlink(path);
            return -1;
        }
