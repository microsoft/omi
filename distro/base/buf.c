...
MI_Result Buf_PackPtrdiff_t (
    ...
    ptrdiff_t data)
{
    if (sizeof(ptrdiff_t) == sizeof(MI_Uint64))
    {
        MI_RETURN_ERR(Buf_PackU64(self, (MI_Uint64)data));
    }
    else if (sizeof(ptrdiff_t) == sizeof(MI_Uint32))
    {
    ...
    MI_Uint32 size)
{
    ...
    if (size)
    {
        ...
        for (i = 0; i < size; i++)
        {
            ...
            n = ((ptrdiff_t)Zlen(data[i]) + 1) * sizeof(ZChar);
        ...
        for (i = 0; i < size; i++)
        {
            ...
            if (i < MI_COUNT(sizes))
                ...
                n = ((MI_Uint32)Zlen(data[i]) + 1) * sizeof(ZChar);
    ...
    MI_Uint32* sizeOut)
{
    ...
    if (sizeof (ptrdiff_t) != sizeof (MI_Uint32))
    {
    ...
    */
    ...
    /* Update the offset */
    self->offset = offset + (((ptrdiff_t)data[size]) - ((ptrdiff_t)data[0]));
...
MI_Result Buf_AppStr(
    ...
    const ZChar* str)
{
    return Buf_App(self, str, Zlen(str) * sizeof(ZChar));
}
...
MI_Result Buf_AppStrN(
    ...
    size_t size)
{
    return Buf_App(self, str, size * sizeof(ZChar));
}
