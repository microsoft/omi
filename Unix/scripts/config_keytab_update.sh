...
configure()
{
    ...
    if [ $? -eq 0 ]; then
       ...
       [ -f $syskeytab ] && [ \( ! -f $omikeytab \) -o \( $syskeytab -nt $omikeytab \) ] && sleep 3 && $ktstrip $syskeytab $omikeytab >/dev/null 2>&1 || true
       ...
       echo "* * * * * [ -f $syskeytab ] && [ \( ! -f $omikeytab \) -o \( $syskeytab -nt $omikeytab \) ] && sleep 3 && $ktstrip $syskeytab $omikeytab >/dev/null 2>&1 || true" >>$tmpfile
