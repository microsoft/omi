...
static void GenStatikGenLine(FILE* os)
{
    ...
    // contain the following string. The ampersands are separated into a format
    // argument to prevent this file (generator.cpp) from containing that string.
    fprintf(os, "/* %cmigen%c */\n", '@', '@');
}
...
static void GenCppClassSource(const MI_ClassDecl* cd)
{
    ...
    // only if we are updating existing one
    if (append)
    {
        bool patched = false;
