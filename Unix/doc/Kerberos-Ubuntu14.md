# Detail Steps To Configure Kerberos Client on Ubuntu 14.04

## Update Software Repos

```
root@omi-ub14x64-zh  # apt-get update -y
```
...
```
apt-get -y install realmd sssd sssd-tools samba-common krb5-user packagekit samba-common-bin samba-libs adcli ntp libkrb5-dev
```
...
2. Add an entry for DNS1, DNS2 and DOMAIN in /etc/resolvconf/resolv.conf.d/head
...
3. Restart server or run below command
...
5. Add configure message to /etc/realmd.conf 
...
6. Edit /etc/samba/smb.conf and add following message
...
7. Edit /etc/pam.d/common-session

```
echo 'session required pam_mkhomedir.so skel=/etc/skel/ umask=0077'>>/etc/pam.d/common-session
```

8. Edit the fallback_homdir in /etc/sssd/sssd.conf like below
...
```
/opt/omi/bin/omicli --hostname omi-ub14x64-01--auth Kerberos -u kdc_account@CONTOSO.COM -p kdc_account_password --port 5985 gi root/cimv2 { MSFT_President Key 1 }

```

2. Sample
```
root@omi-ub14x64-01 # /opt/omi/bin/omicli --hostname omi-ub14x64-01 --auth Kerberos -u kdc_account@CONTOSO.COM -p kdc_account_password --port 5985 gi root/cimv2 { MSFT_President Key 1 }
