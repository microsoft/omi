    ...
    const MI_Filter* filter)
{
	...
	DemoTest_Set_Country(&demoTest,MI_T("USA."));
...
void MI_CALL DemoTest_Invoke_ChangeState(
    DemoTest_Self* self,
    MI_Context* context,
    const MI_Char* nameSpace,
    const MI_Char* className,
    ...
    const DemoTest* instanceName,
    const DemoTest_ChangeState* in)
{
    DemoTest demoTest;
	DemoTest_Construct(&demoTest,context);
	DemoTest_Set_Name(&demoTest,MI_T("EnumeratteInstances has been implemented in updated files."));
	DemoTest_Set_Country(&demoTest,MI_T("USA."));
	DemoTest_Post(&demoTest, context);
    MI_PostResult(context, MI_RESULT_OK);
}
